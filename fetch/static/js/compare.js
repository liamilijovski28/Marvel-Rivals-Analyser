...
const overallStats = {
  ...
  friend: {"KDA Ratio": 2.13, "Kills": 12282, "Deaths": 14443, "Assists": 17048, "Damage": 685096, "Healing": 340489, "Damage Blocked": 280571, "Max Kill Streak": 41, "MVPs": 159, "SVPs": 20}
};
...
function buildOverallBox(titleText, data, compare){
  ...
  statNames.forEach(stat=>{
    ...
    if(data[stat] == compare[stat]){

      val.classList.add('tie-highlight')
    }
  ...
  function makeSide(titleArr, statsObj, compareObj){
    ...
    namesArr.forEach(name=>{
      ...
      ['Matches','WinPct','KDA'].forEach(key=>{
        ...
        else if(statsObj[name][key] == compareObj[name][key]){
